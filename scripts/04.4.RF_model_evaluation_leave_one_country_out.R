...
leave_one_country_models <- function(my_country){
  ...
  cty_fit0 <- fields::Tps(cbind(test_set_xy$x, test_set_xy$y),  # with X and Y only
                          test_set_xy$farm_area_ha, lon.lat = T)
  
  cty_fit1 <- fields::Tps(
    x = as.matrix(test_set_xy[, c('x', 'y')]),
    test_set_xy$farm_area_ha, 
  ...
  test_set_xy$pred_tps_xy  <- as.numeric(predict(
    cty_fit1, test_set_xy[, c('x', 'y')],
    Z = as.matrix(test_set_xy[, c('cropland', 'cattle', 'pop', 'cropland_per_capita',
                                  'sand', 'slope', 'temperature',
                                  'rainfall', 'market', 'maizeyield')]
    )
  ...
  print(rf_country_ref)
  test_set_xy$pred_reference <- as.numeric(predict(rf_country_ref, test_set_xy)  )
  rsq_reference <- round(cor(test_set_xy$farm_area_ha, test_set_xy$pred_reference)^2, 2)
  print(paste0('rsq_reference = ', round(rsq_reference, 2)))
  rsq_ref_cv <- rf_country_ref$results |>
    as.data.frame() |>
    select(Rsquared) |>
    pull() |>
    mean() |>
    round(2)
  print(paste0('rsq_reference_cv = ', rsq_ref_cv))
  ...
  cor_rf_vs_tps <- with(test_set_xy, round(cor(pred_tps_xy, pred_rf, use = 'complete.obs'), 2))   # Get the r2
  rsq_rf_vs_tps <- with(test_set_xy, round(cor(pred_tps_xy, pred_rf, use = 'complete.obs')^2, 2))   # Get the r2
  
  # retrieve variable importance
  var_imp <- rf_country_ref$finalModel$variable.importance |>
    as_tibble() |> 
    mutate(variable = names(rf_country_ref$finalModel$variable.importance)) |> 
    arrange(value) |>
    select(variable, value)
  one_var_imp <- cbind.data.frame(
    country = my_country, 
  ...
  one_rsq <- cbind.data.frame(
    ...
    rsq_reference = rsq_reference,  # RF trained and tested on my_country, calculated R2
    rsq_ref_cv = rsq_ref_cv,        # RF trained and tested on my_country, cross-validated R2
    rsq_rf = rsq_rf,                # RF trained on other countries and tested on my_country
    rsq_rf_vs_tps  = rsq_rf_vs_tps, # R2 for predictions from Tps vs RF trained on other countries
    cor_rf_vs_tps = cor_rf_vs_tps   # correlation coef for predictions from RF vs Tps
  )
  mult_rsq <- rbind.data.frame(mult_rsq, one_rsq)
  mult_var_imp <- rbind.data.frame(mult_var_imp, one_var_imp)
  
  results <- list(mult_rsq, 
                  cty_fit,
                  rf_country_model
                  )
  assign(paste0('varimp_table_for_', my_country), mult_var_imp, envir = .GlobalEnv)
...
deb <- Sys.time()
mult_rsq <- mult_var_imp <- data.frame()
