# Step 1: evaluate the predicted distributions per grid cell against observed empirical CDF of grid cells that have more than 10 obs
# Step 2: evaluate the goodness-of-fit of theoretical distributions against conditional distributions
# Step 3: Find parms of a theoretical distribution to describe farm size per cell
# Step 4: Draw random samples along 
# maybe better to check bayesian tools that allow prediction of the distribution, not just mean + std error
...
# calc_nb_farms <- terra::rast('../data/processed/nb_farms_per_grid_cell.tif') 
# calc_nb_farms <- calc_nb_farms * terra::resample(mask_forest_ssa, calc_nb_farms) # 70 millions farms if these 2 lines are skipped
# calc_nb_farms <- calc_nb_farms * terra::resample(mask_drylands_ssa, calc_nb_farms)
...
all_cells <- cell_id |>
  ...
  summarize(nb_farms = n(), actual_farm_sizes = list(farm_area_ha)) 
...
comp_distr <- cell_quantiles |>
  mutate(ks_D = ks.test(unlist(actual_farm_sizes), unlist(pred_farm_sizes))$statistic,
         ks_pval = ks.test(unlist(actual_farm_sizes), unlist(pred_farm_sizes))$p.value)
...
# may try out Gamma Distribution, Inverse Gaussian Distribution later on
set.seed(2024)
...
theor_farms <- my_points_cells |>
  ...
  summarize(pred_farm_sizes = map(list(across(starts_with('qrf_q'))), unlist)) |>
  mutate(virt_farms = map(pred_farm_sizes, \(x) approx(x = 0.01 * 1:100, y = unlist(x), xout = sort(runif(nb_farms)), rule = 2)$y),
...
count_r80 <- 0
for(i in sort(1:nrow(theor_farms))){
  print(paste0('------------- row ', i, '----------------' ))
  a <- theor_farms$pred_farm_sizes[[i]]
  b <- quantile(theor_farms$virt_farms_f_max_trunc[[i]], 0.01 * 1:100)
  c <- quantile(theor_farms$virt_farms[[i]], 0.01 * 1:100)
...
save(theor_farms, theor_farms_application, file = '../data/processed/fsize_distribution_resample_long.rdata')
...
# limit the usage of RAM by terra package to 80%. may need to remove large objects from global env (qf_model)
gc()
terra::terraOptions(memfrac = 0.8, todisk = T, verbose = F)
...
selected_theor_app <- theor_farms |>
  ...
  unnest_longer(fitted_trunc_logn)
...
pal <- colorRampPalette(c('skyblue1', 'blue4'))
P01 <- ggplot(selected_theor_app, aes(fitted_trunc_logn, colour = paste0(x, ', ', y))) + 
  stat_ecdf(geom = 'line', linewidth = 0.8) +
  ...
  scale_colour_manual(values = pal(15)[3:14]) +
  theme_test() +
...
pal <- colorRampPalette(c('darkgreen', 'darkolivegreen3', 'gold', 'orange', 'darkred' ))
...
terra::plot(ssa, col = 'azure', main = "Kolmogorov's D for a log-normal distribution \nfitted to farm sizes across SSA",
            panel.first = grid(col = "gray", lty = "solid"), pax = list(cex.axis = 1.4), mar  =  c(5, 4, 4, 3.5))
terra::plot(theor_farms_rast$ks_D, col = terrain.colors(100), cex = 1, axes  =  F, add = T)
terra::plot(ssa, axes = F, add = T)
dev.off()
