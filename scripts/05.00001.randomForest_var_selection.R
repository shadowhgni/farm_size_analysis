...
# TRY the QRF model
# qrf_model <- list(
#   label = 'customized quantile random forest',
#   type = 'Regression',
#   library = 'quantregForest',
#   loop = NULL,
#   parameters = data.frame(parameter = c('mtry', 'ntree', 'splitrule', 'min.node.size'), 
#                           class = c('numeric', 'numeric', 'character', 'numeric'), 
#                           label = c('mtry', 'ntree', 'splitrule', 'min.node.size')),
#   grid = function(x, y, len = 3, search = 'grid') {
#     out <- expand.grid(
#       mtry = unique(sample(1:ncol(x), size = len, 
#                            replace = TRUE)),
#       ntree = 1500,
#       splitrule = c('variance', 'extratrees'),
#       min.node.size = c(5, 50, 55, 100, 200)
#     )
#     out
#   },
#   fit = function(x, y, wts, param, lev, last, classProbs, ...) {
#     quantregForest::quantregForest(x, y, mtry = param$mtry, ntree = param$ntree, 
#                                    splitrule = param$splitrule, nodesize = param$min.node.size, ...)
#   },
...
tune_grid <- expand.grid(
  mtry = c(2, 4, 6, 8),                              # I first tried 1:8 and 4 was the best; then I tried 4:5, 4 was still the best. I think of it as a local optimum pb
  splitrule = c('variance', 'extratrees'),           # I tried c('variance', 'extratrees'); extratrees was the best
...
qrf_full_model <- caret::train(
  ...
  trainControl = train_control,
  quantreg = T,
  keep.inbag = T,  # check if this works, just like savePredictions in the control
  tuneGrid = tune_grid,
  importance  = 'permutation',
  num.trees = 1500
) 
...
parallel::stopCluster(cl)
save(qrf_model, file = '../data/processed/2024-10-14.qrf_full_model.rdata')
fin <- Sys.time() - deb; print(fin)
