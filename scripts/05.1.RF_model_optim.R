...
# ------------------------------------------------------------------------------
	seeds_list <- as.list(c(x[,paste0("seed_", 1:10)], sample(10000, 1)))
...
seeds <- sapply(1:10, \(i) sample.int(10000, size = nrow(tune_grid)))
colnames(seeds) <- paste0("seed_", 1:10)
tune_grid <- cbind(tune_grid, seeds)

mbuck <- c(1, 5, seq(10, 100, 10), 200)
tune_grid <- data.frame(tune_grid[rep(1:nrow(tune_grid), each=length(mbuck)), ], mbuck=mbuck)
